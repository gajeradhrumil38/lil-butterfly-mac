...
final class ScreenOverlay {
    ...
    func visit(message: String) {
        guard !isBusy, let host = window.contentView else { return }
        isBusy = true

        let edge = ScreenEdge.allCases.randomElement()!
        let margin: CGFloat = 60
        let rest = restPoint(for: edge, margin: margin)
        let off = offscreenPoint(for: edge, restX: rest.x, restY: rest.y, margin: margin)

        let s = ButterflyView.size
        let butterfly = ButterflyView(frame: CGRect(x: off.x - s / 2, y: off.y - s / 2, width: s, height: s))
        host.addSubview(butterfly)
        butterfly.layer?.position = off
        ...
        func positionBubble(near point: CGPoint) {
            ...
            let x = onRightHalf ? point.x - s / 2 - bubble.frame.width - 10 : point.x + s / 2 + 10
    ...
    private func restPoint(for edge: ScreenEdge, margin: CGFloat) -> CGPoint {
    ...
    private func offscreenPoint(for edge: ScreenEdge, restX: CGFloat, restY: CGFloat, margin: CGFloat) -> CGPoint {
        let w = screenFrame.width
        let h = screenFrame.height
        switch edge {
